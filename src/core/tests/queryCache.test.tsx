...
describe('queryCache', () => {
  ...
  test('removeQueries does not crash when exact is provided', async () => {
    ...
    expect(() => removeQueries(environment, key, { exact: true })).not.toThrow()
  ...
  test('QueriesObserver should update when a query is removed', async () => {
    ...
    await sleep(1)
    expect(findQuery(testEnvironment, key1, { active: true })).toBeUndefined()
    expect(findQuery(testEnvironment, key2, { active: true })).toBeDefined()
    unsubscribe()
    expect(findQuery(testEnvironment, key1, { active: true })).toBeUndefined()
    expect(findQuery(testEnvironment, key2, { active: true })).toBeUndefined()
    testCache.clear()
    expect(results.length).toBe(4)
    expect(results).toMatchObject([
      [{ data: undefined }, { data: undefined }],
  ...
  test('findAll should filter correctly', async () => {
    ...
    expect(findQueries(testEnvironment, key1, { active: false })).toEqual([
      query1,
    ])
    expect(findQueries(testEnvironment, key1, { active: true })).toEqual([])
    expect(findQueries(testEnvironment, key1, { stale: true })).toEqual([])
    expect(findQueries(testEnvironment, key1, { stale: false })).toEqual([
      query1,
    ])
    expect(
      findQueries(testEnvironment, key1, { stale: false, active: true })
    ).toEqual([])
    expect(
      findQueries(testEnvironment, key1, { stale: false, active: false })
    ).toEqual([query1])
    expect(
      findQueries(testEnvironment, key1, {
        stale: false,
        active: false,
        exact: true,
      })
    ...
    expect(findQueries(testEnvironment, key2, { stale: undefined })).toEqual([
      query2,
    ])
    expect(findQueries(testEnvironment, key2, { stale: true })).toEqual([
      query2,
    ])
    expect(findQueries(testEnvironment, key2, { stale: false })).toEqual([])
    expect(findQueries(testEnvironment, [{ b: 'b' }])).toEqual([query3])
    expect(
      findQueries(testEnvironment, [{ a: 'a' }], { exact: false })
    ).toEqual([query3])
    expect(findQueries(testEnvironment, [{ a: 'a' }], { exact: true })).toEqual(
      []
    )
    expect(
      findQueries(testEnvironment, [{ a: 'a', b: 'b' }], { exact: true })
    ).toEqual([query3])
    ...
    expect(
      findQueries(testEnvironment, [{ a: 'a' }], { stale: false })
    ).toEqual([query3])
    expect(findQueries(testEnvironment, [{ a: 'a' }], { stale: true })).toEqual(
      []
    )
    expect(
      findQueries(testEnvironment, [{ a: 'a' }], { active: true })
    ).toEqual([])
    expect(
      findQueries(testEnvironment, [{ a: 'a' }], { inactive: true })
    ).toEqual([query3])
    expect(
